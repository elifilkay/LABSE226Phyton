...
def passGen(num):
    while num < 3 or num > 7:
        print("User number must be between 3 and 7, inclusive.")
        num = int(input("Enter the number of words to choose (between 3 and 7, inclusive): "))

    chosen_words = random.sample(words, num)
    password = ""
...
user_number = int(input("Enter the number of words to choose (between 3 and 7, inclusive): "))
print(passGen(user_number))
print("******************")
...
def rep_with_upper(string):
    ...
    char_to_replace = random.choice(string)
    new_string = string.replace(char_to_replace, char_to_replace.upper())
    return new_string
...
def swap_letters(string):
    ...
    if len(string) < 2:
        ...
        middle_chars = string[2:-2]
        new_string = last_two_chars + middle_chars + first_two_chars
        return new_string
...
print("New password after rep_with_upper: " + new_password)
...
def search_letter(source, key_letter):
    if key_letter in source:
        return True
    else:
        return False

key_letter = input("Enter your name's first letter: ")
result = search_letter(final_password, key_letter)
if result:
    print("The key letter is in the final password.")
else:
    print("The key letter is not in the final password.")
